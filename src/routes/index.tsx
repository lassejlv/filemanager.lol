...
function RouteComponent() {
  ...
  if (!objects) return Error({ error: { message: 'No objects found', name: 'No objects found' } })
    ...
    <div className='p-4'>
      <div className='flex justify-end items-center mb-4 sticky top-0  w-full z-10 bg-transparent backdrop-blur-lg '>
        <div className='flex gap-2 justify-end mt-1'>
          ...
          <Button variant='ghost' size='icon' onClick={() => refetch()} disabled={isFetching || loading || isRefetching}>
        ...
        {objects.map((object) => (
          <ContextMenu key={object.path}>
                  ...
                  <Button asChild variant='ghost' className='h-auto w-full justify-start p-4'>
                    <a>
                      <div className='shrink-0'>
                        {object.is_dir ? <Folder size={24} /> : <></>}

                        {!object.is_dir ? <IconProvider ext={object.ext} /> : <></>}
                      </div>
            ...
            <ContextMenuContent>
              ...
              >
                <Trash /> Delete {object.is_dir ? 'Folder' : 'File'}
              </ContextMenuItem>
              <ContextMenuItem onClick={async () => message('Not yet implemented', { kind: 'info' })}>
